...
def chat(message):
    ...
    try:
        ...
        response = modelB.generate_content('hello, ' + userMsg + 'answer in less than 200 words').text
        bot.reply_to(message, response)
    except:
        chatLog.exception('Model reply is too long to send by the bot')
        bot.reply_to(message, 'I dont wanna reply... its my rest time dude!')
